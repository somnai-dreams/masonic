 ...
 * to `columnGutter`).
 */
const createPositioner = (columnCount, columnWidth, columnGutter = 0, rowGutter = columnGutter, precomputedSizes // hacky addition to make the first row items size correctly on first render
) => {
  // O(log(n)) lookup of cells to render for a given viewport size
  // Store tops and bottoms of each cell for fast intersection lookup.
  const intervalTree = (0, _intervalTree.createIntervalTree)();
